...
function git_clone() {
	launch="cloning blackshark";splashdown="blackshark cloned";echo
	[ -d "${main}" ] && rm -rf ${main} > /dev/null 2>&1
	(git clone https://github.com/Athexhacker/blackshark --quiet ${main} --depth=1) & spinner
}
...
if [ ${OS} != "DEBIAN" ]; then
	clear
	dialog --title "©blackshark" --inputbox  "*Ngrok Authtoken verification \nEnter your authtoken without './' :" 0 0 2> authtk
        mv authtk ${BIN}
        chmod +x ${BIN}/authtk
        sleep 2
        clear
        ...
        sleep 1       
        printf "\n${green}>>${white} simply run : blackshark \n\n ${nc}"
else 
        clear
        dialog --title "© blackshark 2024-2025" --inputbox  "*Ngrok Authtoken verification \nEnter your authtoken without './' :" 0 0 2> authtk
        mv authtk ${BIN}
        chmod +x ${BIN}/authtk
        sleep 2
        clear
